...
async def recognize_bird(
    ...
    try:
        ...
        # 为所有候选鸟检索资料注入上下文
        rag_context = retrieve_context_for_birds(
            [b["name"] for b in all_metas],
            max_chars=600
        )
...
【权威图鉴资料库】（你必须严格依据以下资料进行识别，禁止编造）：
{rag_context}

【绝对强制要求】：
1. 必须基于拍摄地点的地理位置（国家、城市、生境）进行交叉验证。
2. 优先查阅官方图鉴列表：[{pokedex_names}]。如果识别出的鸟类在这个列表中，你必须【一字不差】地输出列表中的名字！
3. 如果该鸟类完全不在上述列表中，请严格使用《世界鸟类名录》中的标准中文学名，绝对禁止生造词汇、使用地方俗名或笼统称（如"八哥"、"麻雀"）。
...
{{
  "top5_candidates": [
    {{
      ...
      "reason": "结合图片中的外貌羽色、体型等视觉特征的识别理由，并结合解析出的地理位置（国家/城市）进行解释"
    }}
  ],
