...
import { MapPin, Crosshair, Loader2, CheckCircle2 } from "lucide-react"; // 🌟 引入新图标
import {
  Dialog,
  DialogContent,
  DialogTitle,         
...
export default function Home() {
  ...
  const [coords, setCoords] = useState<{lat: number, lng: number} | null>(null); // 🌟 新增：存储经纬度
  const [isLocating, setIsLocating] = useState(false); // 🌟 新增：定位加载状态
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // 🌟 核心修改：放弃临时的 Blob 链接，将文件读取为永久的 Base64 字符串
      const reader = new FileReader();
      reader.onloadend = () => {
        // reader.result 就是一段包含图片所有像素信息的超长文本 (data:image/jpeg;base64,...)
            ...
            <div className="flex flex-col lg:flex-row max-h-[90vh] overflow-y-auto lg:overflow-hidden custom-scrollbar-dark">
              <div className="relative h-72 lg:h-auto lg:w-2/5 flex-shrink-0">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-emerald-950 via-emerald-950/20 to-transparent lg:bg-gradient-to-r lg:from-transparent lg:to-emerald-950 flex flex-col justify-end p-8 lg:p-10">
                  ...
                  <p className="text-lime-400 font-bold italic mt-3 text-xl drop-shadow-md">
                    {selectedBird.englishName}
                  </p>
                  <p className="text-emerald-300 font-medium mt-1 text-sm opacity-80">
                    {selectedBird.latinName}
                  </p>
                </div>
                
                <div className="absolute top-6 right-6 lg:left-6 lg:right-auto bg-black/40 backdrop-blur-md border border-lime-500/30 text-lime-100 px-5 py-2 rounded-full text-sm font-black shadow-xl">
                  {selectedBird.family}
                </div>
              ...
              <div className="p-8 lg:p-12 lg:w-3/5 lg:overflow-y-auto custom-scrollbar-dark space-y-8">
                <div className="bg-emerald-900/40 p-6 md:p-8 rounded-3xl shadow-inner border border-emerald-700/50 relative transform hover:scale-[1.01] transition-transform">
                  ...
                  <p className="text-emerald-100 mt-2 text-base md:text-lg leading-relaxed tracking-wide font-medium">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-gradient-to-br from-yellow-900/40 to-lime-900/40 p-6 md:p-8 rounded-3xl shadow-inner border border-yellow-700/50 relative hover:from-yellow-800/40 transition-colors">
                    ...
                    <p className="text-yellow-100/90 mt-2 text-base md:text-lg leading-relaxed font-bold tracking-wide">
