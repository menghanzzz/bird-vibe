...
// 🔧 修改：BirdImage 只显示官方图片，不再优先显示用户实拍
function BirdImage({
  name,
  englishName,
  latinName,
  ...
  useEffect(() => {
    ...
    // 🆕 优先使用已存入的官方图片 URL（识别时从 iNaturalist 获取的）
    if (officialImageUrl) {
      setImgSrc(officialImageUrl);
      setIsError(false);
      return;
    }

    const fetchBirdImage = async () => {
      // 1. iNaturalist API（优先用拉丁学名查，最准）
      ...
      // 2. Wikipedia fallback（只用英文名查，且做严格校验）
  ...
  }, [name, englishName, latinName, officialImageUrl]);
  ...
  return (
    <img
      ...
      className={`w-full h-full object-cover transition-all duration-700 ${
        !isUnlocked
          ? "brightness-[0.7] saturate-[0.5] contrast-[0.8]"
          : "saturate-[1.1]"
      }`}
      alt={name || "未知鸟类"}
    />
...
export default function App() {
  ...
  // 合并 baseBirds 和 customBirds，去重并补全字段
  const customMap = new Map(customBirds.map((c) => [c.id, c]));
  const allBirds = baseBirds
    .map((b) => {
      ...
      if (custom) {
        return {
          ...b,
          ...custom,
          englishName: b.englishName || custom.englishName,
          latinName: b.latinName || custom.latinName,
        };
      }
      return b;
    })
    .concat(
      customBirds.filter(
        (c) => !baseBirds.some((b) => b.id === c.id || b.name === c.name)
      )
    );

  const filteredBirds = allBirds.filter(
    (b) => activeCategory === "全部" || b.category === activeCategory
  );
    ...
    if (bird && !birdRagDetails[bird.name]) {
      try {
        ...
        if (res.ok) {
          ...
          setBirdRagDetails((prev) => ({
            ...prev,
            [bird.name]: {
              details: {
                appearance: `暂无${bird.name}的详细百科资料。这只鸟是通过 AI 识别加入你的图鉴的，快去观察它、记录它的故事吧！`,
                habitatAndHabits: "通过实地观察来补充...",
                callCharacteristics: "留意它的鸣叫声...",
                distribution: "在你的记录地点附近...",
              },
            },
          }));
        }
      } catch (e) {
        console.error("获取鸟类详情失败:", e);
      }
    }
  ...
  return (
    <main className="min-h-screen bg-zinc-50 p-6 md:p-10 font-sans relative">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredBirds.map((bird) => {
            const isUnlocked =
              unlockedIds.includes(bird.id) ||
              customBirds.some((b) => b.id === bird.id);
            const isFlipped = flippedId === bird.id;
            const myRecord = birdRecords[bird.id] || birdRecords[bird.name];

            return (
              <div
                key={bird.id}
                className="relative h-[440px] w-full [perspective:1000px]"
              >
                <div
                ...
                >
                  ...
                  <div className="absolute inset-0 [backface-visibility:hidden] bg-white p-5 rounded-3xl border-2 border-zinc-100 shadow-sm flex flex-col">
                    <div className="h-40 w-full rounded-2xl overflow-hidden relative mb-4 bg-zinc-100 flex-shrink-0">
                      {/* 🔧 修改：只传 officialImageUrl，不传用户实拍照片 */}
                      <BirdImage
                        name={bird.name}
                        englishName={bird.englishName}
                        latinName={bird.latinName}
                        isUnlocked={isUnlocked}
                        officialImageUrl={(bird as any).officialImageUrl}
                      />
                      {!isUnlocked && (
                    ...
                    <div className="flex-1 flex flex-col items-center text-center">
                      ...
                      <div className="text-[11px] font-bold mt-1 space-y-0.5 flex flex-col items-center text-emerald-600">
                        ...
                        <p className="italic text-emerald-500/70">
                          {bird.latinName || (
                    ...
                    <div className="mt-4 pt-4 border-t border-zinc-100">
                      {isUnlocked ? (
                        <p className="text-[10px] font-bold text-zinc-500 text-center line-clamp-2">
                          {bird.funFact}
                        </p>
                      ) : (
                        <div className="flex flex-col gap-2">
                          <div className="bg-emerald-50 text-emerald-700 text-[9px] font-black py-1 px-2 rounded-md border border-emerald-100 text-center uppercase tracking-widest">
                            尚未解锁
                          </div>
                          <button
                            onClick={(e) => {
                            ...
                            className="w-full py-2 bg-emerald-600 text-white text-[10px] font-black rounded-lg hover:bg-emerald-700 transition-colors"
                          >
                            去识别
                          </button>
                        </div>
                  ...
                  <div className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)] bg-emerald-900 p-5 rounded-3xl border-2 border-emerald-800 shadow-lg flex flex-col text-white">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-3 scrollbar-hide">
                      <div className="bg-white/10 p-3 rounded-xl">
                        <div className="flex items-center gap-2 text-[10px] font-bold text-emerald-200 mb-1">
                          <Calendar className="w-3 h-3" />
                          首次相遇
                        </div>
                        <p className="text-sm font-bold">
                          {myRecord?.firstDate || "未记录"}
                        </p>
                      </div>
                      <div className="bg-white/10 p-3 rounded-xl">
                        <div className="flex items-center gap-2 text-[10px] font-bold text-emerald-200 mb-1">
                        ...
                        <p className="text-sm font-bold">
                          <LocationDisplay
                            ...
                            lng={myRecord?.lng}
                            fallback={
                              myRecord?.firstLocation || bird.location
                            }
                          />
                        </p>
                      </div>
                      <div className="bg-white/10 p-3 rounded-xl">
                        <div className="flex items-center gap-2 text-[10px] font-bold text-emerald-200 mb-2">
                          ...
                          <div className="grid grid-cols-3 gap-1.5">
                            {myRecord.photos.map((url, idx) => (
                              <img
                                key={idx}
                                src={url}
                                className="w-full h-14 object-cover rounded-lg border border-white/20"
                                onError={(e) => {
                                  (
      ...
      {detailBird && (
        ...
        >
          <div
            className="bg-[#fdfbf7] w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl border border-[#e6dcd0] p-6 relative"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setDetailBird(null)}
              className="absolute top-4 right-4 p-2 bg-white rounded-full shadow-sm border border-[#e6dcd0] hover:bg-[#f5f0e8] transition-colors"
            >
              <X className="w-4 h-4 text-[#5c5044]" />
            </button>
            <div className="w-20 h-1 bg-[#e6dcd0] rounded-full mx-auto mb-6" />
            <div className="text-center mb-6">
              <p className="text-[10px] font-black text-amber-700/50 tracking-[0.2em] uppercase mb-2">
                NATURAL NOTE
              </p>
              <h2 className="text-2xl font-black text-[#2c221a] mb-1">
                {detailBird.name}
              </h2>
              <p className="text-sm font-bold text-[#8c7f70] italic">
                {detailBird.englishName || "—"}
              </p>
              <p className="text-xs text-[#b0a396] font-bold mt-1">
                {detailBird.latinName || "—"}
              </p>
              <div className="flex justify-center gap-3 mt-3">
                <span className="px-3 py-1 bg-emerald-100 text-emerald-800 text-[10px] font-black rounded-full">
                  🌿 {detailBird.category}
                </span>
                <span className="px-3 py-1 bg-amber-100 text-amber-800 text-[10px] font-black rounded-full">
                  ⭐ {detailBird.rarity}
                </span>
              </div>
            </div>
            <div className="space-y-4">
              <div className="bg-white/60 p-4 rounded-2xl border border-[#e6dcd0]/80 shadow-sm">
                <h4 className="font-black text-base text-[#2c221a] mb-2">
                  🪶 外观特征
                </h4>
                <p className="text-[#5c5044] text-[13px]">
                  {(() => {
                    const rag = birdRagDetails[detailBird.name];
                    if (!rag) return "🌿 正在从自然笔记中调取资料...";
                    return (
                      rag.details?.appearance ||
                      `${detailBird.name}的详细外观描述正在整理中...`
                    );
                  })()}
                </p>
              </div>
              <div className="bg-white/60 p-4 rounded-2xl border border-[#e6dcd0]/80 shadow-sm">
                ...
                <p className="text-[#5c5044] text-[13px]">
                  {(() => {
                    ...
                    if (!rag) return "🌿 正在从自然笔记中调取资料...";
                    return (
                      rag.details?.habitatAndHabits ||
                      `它常出现在${
                        detailBird.location || "你记录的地方"
                      }附近...`
                    );
                  })()}
                </p>
              ...
              <div className="bg-white/60 p-4 rounded-2xl border border-[#e6dcd0]/80 shadow-sm">
                ...
                <p className="text-[#5c5044] text-[13px]">
                  {(() => {
                    ...
                    if (!rag) return "🌿 正在从自然笔记中调取资料...";
                    return (
                      rag.details?.callCharacteristics ||
                      "留意它的独特鸣声..."
                    );
                  })()}
                </p>
